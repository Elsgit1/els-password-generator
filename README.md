# els-password-generator

A password/passphrase generator web app, packaged as a container and deployed to Kubernetes.

## What's in this repo

- **`app/`** - the React app (built with Vite). This is the actual product.
- **`.github/workflows/`** - the pipeline that builds and publishes the container image.
- **`k8s-deployment/`** - the Kubernetes manifests that describe how the app runs in the cluster.
...
2. **Update the manifest** - The pipeline then automatically updates `k8s-deployment/deployment.yaml` with the new image tag and opens a pull request with that change.
